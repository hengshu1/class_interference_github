...
if __name__ == '__main__':
    ...
    # net = load_model(net, 'vgg')
    # net.eval()
    # measure_net('vgg', net.features[0], net, 64, 10, mode='train')
    # measure_net('vgg', net.features[0], net, 64, 10, mode='test')

    # net = ResNet18().to(device)
    # net = load_model(net, 'resnet18')
    # net.eval()
    # measure_net('resnet18', net.conv1, net, 64, 10, mode='train')
    # measure_net('resnet18', net.conv1, net, 64, 10, mode='test')

    # net = GoogLeNet().to(device)
    # net = load_model(net, 'googlenet')
    # net.eval()
    # measure_net('googlenet', net.pre_layers, net, 192, 10, mode='train')
    # measure_net('googlenet', net.pre_layers, net, 192, 10, mode='test')

    # net = DLA().to(device)
    # net = load_model(net, 'dla')
    # net.eval()
    # measure_net('dla', net.base, net, 16, 10, mode='train')
    # measure_net('dla', net.base, net, 16, 10, mode='test')
...
def test_broadcast():
    ...
    # total = torch.ones(means.shape[0]) * 3
    total = torch.rand(means.shape[0])
    print(total)
    print('total.shape=', total.shape)
    # print(means/total)#hot allowed
    print(means/total[:, None])
